...
describe('LineItemDetailPage', () => {
    ...
    it('requires confirmation before deleting an unassigned manual line item', async () => {
        renderLineItemDetail();

        await waitFor(() => expect(screen.getByRole('button', { name: 'More actions' })).toBeInTheDocument());
        await userEvent.click(screen.getByRole('button', { name: 'More actions' }));
        await userEvent.click(screen.getByRole('menuitem', { name: 'Delete line item' }));

        expect(mockAxiosInstance.delete).not.toHaveBeenCalled();
        const dialog = screen.getByRole('alertdialog');
        expect(within(dialog).getByText(/permanently deletes/i)).toBeInTheDocument();
        await userEvent.click(within(dialog).getByRole('button', { name: 'Delete line item' }));
    ...
    it('disables synced line item edit and delete actions with tooltip copy', async () => {
        ...
        await userEvent.click(screen.getByRole('button', { name: 'More actions' }));
        expect(screen.getByRole('menuitem', { name: 'Delete line item' })).toBeDisabled();
        ...
        await userEvent.hover(screen.getByRole('menuitem', { name: 'Delete line item' }).parentElement as HTMLElement);
    ...
    it('returns to review after confirming deletion when opened from review', async () => {
