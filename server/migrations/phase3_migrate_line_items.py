...
def determine_transaction_source(mongo_line_item_id: str) -> str:
    """
    Determine transaction source from line item ID.

    Line items are stored as "line_item_{mongo_transaction_id}"
    The prefix tells us which collection the transaction came from.

    Args:
        mongo_line_item_id: MongoDB line item ID

    Returns:
        Source type (venmo, splitwise, stripe, cash, or manual)
    """
    # Line items are formatted as "line_item_{transaction_id}"
    if not mongo_line_item_id.startswith('line_item_'):
        return 'manual'

    # Extract the transaction ID portion
    # This is a heuristic - we'll handle missing transactions as orphans
    return 'unknown'
...
def create_manual_transaction(
    ...
    logging.info(f"  Created manual transaction {txn_id} for orphaned line item {mongo_id}")
    return txn_id
...
def migrate_line_items(
    ...
    for doc in collection.find():
        ...
        if not transaction_id:
            logging.warning(f"Could not find or create transaction for line item {mongo_id}")
            continue
        ...
        try:
            ...
            if count % 100 == 0:
                db_session.commit()
                logging.info(f"  Migrated {count}/{total_items} line items...")
    ...
    logging.info(f"\n{'='*60}")
    logging.info(f"✓ Migrated {count} line items (skipped {skipped} existing)")
    logging.info(f"{'='*60}\n")
...
def verify_migration(db_session, mongo_db):
    ...
    if mongo_count == pg_count:
        ...
        logging.warning(f"✗ Verification failed - counts do not match")

    # Check for orphaned transactions
    manual_txn_count = db_session.query(Transaction).filter_by(source='manual').count()
    if manual_txn_count > 0:
        logging.info(f"  Created {manual_txn_count} manual transactions for orphaned line items")
